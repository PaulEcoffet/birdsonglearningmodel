/*
 * Slightly edited by Paul Ecoffet, using the code from the DYNAMICAL SYSTEMS LAB
...
for(ikp=1;ikp<2;ikp++){
    ...
    ptr4=stdout; /* Output on the standard output. */
		 						 /* ugly but minimise impact on previous code */
...
if(taux==20) {
		/* Does not take data from file but from the standard input steam */
		fscanf(stdin,"%lg %lg",&aa.amplitud,&aa.beta1);
		/* Here, ptr4 is the standard output */
		fprintf(ptr4,"%lg\n",preout*10);
