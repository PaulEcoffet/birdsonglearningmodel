/*
From Boari et al., slightly modified by Paul Ecoffet
The modification only allows the software to be included in scripts
(see gteextractor.sh)
...
int main() {
   int i,j,k,Ndatos;
    ...
    //Loads song
    double *original;
    Ndatos=filesize("workon.dat",1);
    original=dvector(1,Ndatos);
    file_to_vector("workon.dat",original,1,Ndatos,1,1);
    double *sound;
  ...
  // vector_to_file("hilbert.exampleGTE.dat",hilb,1,Ndatos);
   ...
   //vector_to_file("env_suave_new.exampleGTE.dat",sav,1,Ndatos);
   ...
   //vector_to_file("deri.exampleGTE_40000.Sound.dat",deri,1,Ndatos);
   ...
   for(i=1;i<POT;i++){
       if(norm[i]<0.025 && norm[i+1]>0.025) {gte1[++temp]=(double) i; inic[++count]=i;}
       if(norm[i]>0.025 && norm[i+1]<0.025) {gte1[++temp]=(double) i; fin[count]=i;}
   }
   ...
   //Intrasyllabic maxima
    ...
    vector_to_file("gtes1.workon.dat",gte1,1,1000);
    vector_to_file("gtes2.workon.dat",gte2,1,1000);
    vector_to_file("gtes4.workon.dat",gte4,1,1000);
    vector_to_file("gtes3.workon.dat",gte3,1,1000);
