...
def fit_gesture(gesture, samplerate=44100):
    ...
    for k in range(1,i+1):  # prior on exponential
        prior.extend([1/k, 10/k])
        dev.extend([0.05, 0.05])
        mins.extend([-10, 0])
        maxs.extend([10, 10])
    for k in range(1,j+1):
        prior.extend([0.1/k, 0, 200*k**2])
    ...
    x, y, score = hill_climbing(
        lambda x: mfcc(gen_sound(x, len(gesture), alphaf_shape=(i, j)),
        ...
        max_iter=1000, comparison_method=lambda g, c: fastdtw(g, c, dist=2)[0],
        temp_max=10,
        verbose=False)
    return x, score
...
if __name__ == "__main__":
    ...
    for i in range(20):
        try:
            ...
            plt.savefig('res/{}_song.svg'.format(i))
            print(x)
            ...
            plt.legend()
            plt.savefig('res/{}_param.svg'.format(i))
            if score < best_score:
                best_x = x
                best_score = score
